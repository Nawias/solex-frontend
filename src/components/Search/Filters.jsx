...
class Filters extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.location.search != '') {
            axiosInstance.get("/public/podkategorie" + this.props.location.search)
                .then((response) => {
        ...
        if ( params.get("catId")!== null) {
            ...
            axiosInstance.get("/public/getCategory?id=" + params.get("catId"))
                .then((response) => {
                    console.log(response);
    ...
    categoryBar = () => {
        ...
        if (this.state.parents === undefined){
            return <div><Link to={"/"}>Solex</Link></div>
        }else{
            let category = this.state.parents;
            while (true) {
                if (category.parent === null) {

                    link.push(<a href={"/szukaj?query=&catId=" + category.id}>{category.name}</a>);
    ...
    render() {
        ...
        if (this.state.subcategories !== null && this.state.subcategories !== undefined) {
            return (
                <Container>
                        ...
                        <Col xs={9} className={"my-3"}>
                            ...
                            <select name="subcategories" id="" onChange={this.selectCategory}>
                                {this.state.subcategories.map((cat) => (
                                    <option value={"/szukaj?query=&catId=" + cat.id}
                                            selected={(this.state.catId == "?query=&catId=" + cat.id) ? "selected" : ""}>{cat.name}</option>
                                ))};
                            </select>
